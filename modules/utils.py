...
def getConfigDir(defaultConfigDir):
    configDir = defaultConfigDir
    return configDir
    
   
def getPromptTitle(promptTitle):
...
def my_fun(source_dict, menu_completion, plugins, data, path_entry_name="", path_entry_name_path="", tab="\t"):
    for key in source_dict:
        ...
        tmpDir = getTmpDir(default=os.environ['HOME'] + '/.starterTree/')
        def register(p):
            if p.getName() == key:
